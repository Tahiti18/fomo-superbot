...
bot.command('scan', async (ctx) => { const ca=(ctx.match as string||'').trim(); if(!ca) return ctx.reply('Usage: /scan <contract>'); await ctx.reply(`Scanning: ${ca}`); });
bot.command('honeypot', async (ctx) => { const ca=(ctx.match as string||'').trim(); if(!ca) return ctx.reply('Usage: /honeypot <contract>'); await ctx.reply(`Honeypot test: ${ca}`); });
bot.command('meme', async (ctx) => { const p=(ctx.match as string||'').trim(); if(!p) return ctx.reply('Usage: /meme <prompt>'); await ctx.reply(`Meme: ${p} (stub)`); });
...
bot.on('callback_query:data', async (ctx) => {
  ...
  for (const r of router.patterns) {
    if (matchPattern(r.pattern, data)) {
      ...
      // @ts-ignore
      const mod = H[ns];
      if (!mod || !mod[act]) return ctx.answerCallbackQuery({ text: 'Handler not found', show_alert: true });
      await mod[act](ctx);
      return;
    }
  }
