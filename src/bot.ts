...
// Market
bot.command("chart", async (ctx) => H.market.chart?.(ctx) ?? ctx.reply("Chart (soon)"));
bot.command("holders", async (ctx) => H.market.holders?.(ctx) ?? ctx.reply("Holders (soon)"));
bot.command("alerts", async (ctx) => H.market.alerts?.(ctx) ?? ctx.reply("Alerts (soon)") );
